...
def test_valid_minimal_config(write_config: WriteConfig) -> None:
...
def test_comments_ignored(write_config: WriteConfig) -> None:
...
def test_keys_normalized_to_uppercase(write_config: WriteConfig) -> None:
    path = write_config(
        "width=5\n"
        "height=5\n"
        "entry=0,0\n"
...
def test_perfect_boolean_variants(
    write_config: WriteConfig, perfect_val: str, expected: bool,
) -> None:
    path = write_config(
        f"WIDTH=10\nHEIGHT=10\nENTRY=0,0\nEXIT=9,9\n"
        f"OUTPUT_FILE=o.txt\nPERFECT={perfect_val}\n"
...
def test_optional_seed(write_config: WriteConfig) -> None:
    path = write_config(
        "WIDTH=10\nHEIGHT=10\nENTRY=0,0\nEXIT=9,9\n"
        "OUTPUT_FILE=o.txt\nPERFECT=True\n"
        "SEED=42\n"
...
def test_missing_mandatory_key(
    write_config: WriteConfig,
...
def test_line_without_equals(
    write_config: WriteConfig, capsys: CaptureFixture[str],
) -> None:
    path = write_config(
        "WIDTH=10\nBADLINE\nHEIGHT=10\nENTRY=0,0\nEXIT=9,9\n"
        "OUTPUT_FILE=o.txt\nPERFECT=True\n"
...
def test_invalid_width_non_numeric(
    write_config: WriteConfig, capsys: CaptureFixture[str],
) -> None:
    path = write_config(
        "WIDTH=abc\nHEIGHT=10\nENTRY=0,0\nEXIT=9,9\n"
...
def test_invalid_perfect_value(
    write_config: WriteConfig, capsys: CaptureFixture[str],
) -> None:
    path = write_config(
        "WIDTH=10\nHEIGHT=10\nENTRY=0,0\nEXIT=9,9\n"
